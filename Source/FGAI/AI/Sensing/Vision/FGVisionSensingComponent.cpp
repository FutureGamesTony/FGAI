...
UFGVisionSensingComponent::UFGVisionSensingComponent()
{
	...
	TraceParams = FCollisionQueryParams(TEXT("TraceParams"), false, this->GetOwner());
}
...
bool UFGVisionSensingComponent::IsPointVisible(const FVector& PointToTest, const FVector& Origin, const FVector& Direction, float DistanceMinimum) const
{
	...
	if (bIsValid)
	{
		...
		FVector eyeHeightVector = FVector::UpVector * SensingSettings->EyeHeight;
		GetWorld()->LineTraceSingleByChannel(Hit, Origin + eyeHeightVector, PointToTest, ECC_Pawn, TraceParams);
		if (Hit.bBlockingHit)
		{
			UFGVisionSensingTargetComponent* targetComp = Hit.GetActor()->FindComponentByClass<UFGVisionSensingTargetComponent>();
			if (targetComp)
				return bIsValid;

			return FVector::Dist2D(Origin + eyeHeightVector, Hit.Location) > FVector::Dist2D(Origin + eyeHeightVector, PointToTest);
		}
	}
